...
/**
 * Server Action — proxies a customer search to ISS Manager v2 and audits
 * the call. Client_secret never leaves the server. KVKK posture: incoming
 * results are already API-masked; we still log only the query *length*,
 * not the term itself, into audit_events.
 */
import { z } from "zod";
...
async function audit(
  ...
  try {
    ...
    await db.insert(schema.auditEvents).values({
      ...
      sonuc: outcome,
      requestId: `q_len=${query.length};hits=${hits}`,
    });
  } catch (e) {
    logger.error(
...
const HUMAN: Record<string, string> = {
  ...
  auth: "ISS Manager kimlik doğrulaması başarısız. Konfigürasyonu kontrol edin.",
  network: "ISS Manager API'sine erişilemedi. Ağ bağlantısı / VPN kontrol.",
  http: "ISS Manager beklenmeyen bir cevap döndü.",
  parse: "ISS Manager cevabı tanınamadı (yeni alan eklenmiş olabilir).",
};
